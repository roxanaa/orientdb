...
public class OPaginatedCluster extends ODurableComponent implements OCluster {
  ...
  private volatile String                       compressionOptions;
  ...
  public void configure(final OStorage storage, final int id, final String clusterName, final Object... parameters)
    ...
    try {
      ...
      try {
        ...
        final String cfgCompressionOptions = ctxCfg.getValueAsString(OGlobalConfiguration.STORAGE_COMPRESSION_OPTIONS);
        ...
        config = new OStoragePaginatedClusterConfiguration(storage.getConfiguration(), id, clusterName, null, true,
            ...
            cfgCompression, cfgCompressionOptions, null, OStorageClusterConfiguration.STATUS.ONLINE);
      ...
      try {
        ...
        case COMPRESSION:
          ...
          setCompressionInternal(stringValue, ODatabaseRecordThreadLocal.INSTANCE.get().getStorage().getConfiguration()
              .getContextConfiguration().getValueAsString(OGlobalConfiguration.STORAGE_COMPRESSION_OPTIONS));
          break;
          ...
          try {
            ...
            do {
              ...
              if (prevPageRecordPointer >= 0) {
                ...
                try {
                  final OClusterPage prevPage = new OClusterPage(prevPageCacheEntry, false, getChanges(atomicOperation,
                      prevPageCacheEntry));
  ...
  private void updateClusterState(long sizeDiff, long recordsSizeDiff, OAtomicOperation atomicOperation) throws IOException {
    ...
    try {
      OPaginatedClusterState paginatedClusterState = new OPaginatedClusterState(pinnedStateEntry, getChanges(atomicOperation,
          pinnedStateEntry));
  ...
  private void init(final OAbstractPaginatedStorage storage, final OStorageClusterConfiguration config) throws IOException {
    ...
    this.compression = OCompressionFactory.INSTANCE.getCompression(this.config.compression, this.config.compressionOptions);
  ...
  private FindFreePageResult findFreePage(int contentSize, OAtomicOperation atomicOperation) throws IOException {
    ...
    try {
      while (true) {
        ...
        OPaginatedClusterState freePageLists = new OPaginatedClusterState(pinnedStateEntry, getChanges(atomicOperation,
            pinnedStateEntry));
        long pageIndex;
        do {
  ...
  private void updateFreePagesIndex(int prevFreePageIndex, long pageIndex, OAtomicOperation atomicOperation) throws IOException {
    ...
    try {
      ...
      if (nextPageIndex >= 0) {
        ...
        try {
          final OClusterPage nextPage = new OClusterPage(nextPageCacheEntry, false, getChanges(atomicOperation,
              nextPageCacheEntry));
      ...
      if (newFreePageIndex >= 0) {
        ...
        try {
          OPaginatedClusterState clusterFreeList = new OPaginatedClusterState(pinnedStateEntry, getChanges(atomicOperation,
              pinnedStateEntry));
        ...
        if (oldFreePage >= 0) {
          ...
          try {
            final OClusterPage oldFreeLocalPage = new OClusterPage(oldFreePageCacheEntry, false, getChanges(atomicOperation,
